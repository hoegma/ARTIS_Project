...
In~\cite{alashjaee2025machine}, the authors use the VGG16 as a model and initialize it with the weights trained on ImageNet.
...
In addition, the authors use the 140k-real-fake-faces-with-ELA dataset, in which all images have undergone an additional preprocessing step.
This step is error level analysis (ELA), which is a forensic technique 
for examining image segments for varying compression levels 
that arise during digital editing of the image.
...
which contain 3 and 5 convolutional layers and one output layer, respectively.
